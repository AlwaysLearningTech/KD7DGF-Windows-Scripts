<#
.SYNOPSIS
    Install Anytone AT-D578UV CPS (Customer Programming Software)
    
.DESCRIPTION
    Downloads and installs latest Anytone AT-D578UVIII Plus V2 CPS from Bridgecom Systems.
...
.EXAMPLE
    ...
    Install latest D578 CPS
    
.EXAMPLE
    .\Install-AnytoneD578CPS.ps1 -ConfigFile "EmComm-Config.json"
    Install with pre-configured operator details
    
.NOTES
    Requires Administrator privileges
    Logs to C:\Logs\AnytoneD578CPS_Install_YYYYMMDD_HHMMSS.log
    For AT-D578UVIII Plus V2 model only

.INTUNE WIN32 APP DEPLOYMENT
    Install command: powershell.exe -ExecutionPolicy Bypass -NoProfile -File "Install-AnytoneD578CPS.ps1"
    Install behavior: System context
    Detection: File exists %ProgramFiles(x86)%\AnyTone\D578UV_V2\D578UV.exe
...
$LogFile = "$LogDir\AnytoneD578CPS_Install_$Timestamp.log"
...
Write-Log "Starting Anytone AT-D578UV V2 CPS installation"

# Bridgecom download page
$DownloadPage = "https://support.bridgecomsystems.com/anytone-578-v2-model-cps-firmware-downloads"

$TempDir = "$env:TEMP\AnytoneD578CPS"
...
try {
    Write-Log "Fetching Anytone D578 V2 CPS download page"
    ...
    $link = ($html.Links | Where-Object { 
        $_.href -match "D578.*V2.*CPS.*\.zip" -or $_.href -match "578.*CPS.*\.zip"
    ...
    if (-not $link) {
        ...
        $link = "https://support.bridgecomsystems.com/s/article/anytone-578-v2-model-cps-firmware-downloads"
    ...
    Write-Log "Downloading Anytone D578 V2 CPS from $link"
    $ZipPath = "$TempDir\D578_CPS.zip"
    ...
    $Installer = Get-ChildItem -Path $TempDir -Filter "*.exe" -Recurse | 
                 Where-Object { $_.Name -match "setup|install|D578" } | 
    ...
    if ($Process.ExitCode -eq 0) {
        Write-Log "Anytone D578 V2 CPS installed successfully"
    } else {
        Write-Log "Installation returned exit code: $($Process.ExitCode)" -Level WARNING
    }
}
catch {
    Write-Log "Failed to install Anytone D578 V2 CPS: $_" -Level ERROR
...
Write-Log "Anytone AT-D578UV V2 CPS installation completed"
